...
3. 多項式の次数が1の場合は $c_{p,i} = d_{p,i} = 0$, 次数が2の場合は $d_{p,i} = 0$ とする（次数によらず常に12個の係数を格納するための規約）
4. パラメータ $h$ は、曲線の滑らかさの指標として使用され、以下の条件を満たします。

| $h$ | 説明 |
|---|---|
| 0 | 曲線はすべてのブレークポイントで連続 |
| 1 | 曲線はすべてのブレークポイントで連続かつ1階導関数も連続 |
| 2 | 曲線はすべてのブレークポイントで連続かつ1・2階導関数も連続 |

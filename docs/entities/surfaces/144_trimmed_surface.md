...
The `DomainCache` generated by `BuildDomainCache()` is constructed only during the first containment test and is reused thereafter (lazy initialization). If the surface or boundary curves are modified, the cache is invalidated and reconstructed during the next `IsInDomain` call.
...
`ComputeContainmentPolygons` determines the closedness of a boundary loop using a relative tolerance `kCurveClosureRelTolerance` (default 1e-4) in addition to the strict `IsClosed()` check. If the gap between the start and end points is no greater than this value times the curve extent (the AABB diagonal of the sample points), the loop is treated as closed and the domain polygons are constructed (a gap within the tolerance is closed automatically as part of the polygon cycle). This accommodates boundary loops written by real CAD systems, whose seams commonly contain gaps on the order of 1e-6. If a boundary loop cannot be treated as closed, or if it self-intersects, the polygon construction for that boundary throws an exception and `BuildDomainCache()` skips that boundary (no trimming is applied for it, and rendering continues).
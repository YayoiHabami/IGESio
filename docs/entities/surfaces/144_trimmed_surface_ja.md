...
　`ComputeContainmentPolygons`は境界ループの閉性を、`IsClosed()`による厳密判定に加えて相対許容`kCurveClosureRelTolerance`（既定1e-4）を併用して判定する。始終点のギャップがこの値×曲線の広がり（サンプル点のAABB対角長）以下であれば閉曲線とみなし、領域多角形を構築する（許容内のギャップは多角形の閉路として自動的に閉じられる）。実CADが出力する境界ループの継ぎ目には1e-6程度の微小なギャップが存在しうるためである。なお、境界ループが閉曲線とみなせない場合や自己交差を持つ場合、当該境界の多角形構築は例外を送出し、`BuildDomainCache()`はその境界をスキップする（当該境界によるトリムは行われず、描画は継続される）。

...
- [エンティティ](#エンティティ)
  ...
  - [座標系と変換行列](#座標系と変換行列)
    - [座標系](#座標系)
    - [変換行列の適用](#変換行列の適用)
    - [アセンブリ大域変換との合成](#アセンブリ大域変換との合成)
  - [アセンブリとビューの永続化](#アセンブリとビューの永続化)
...
　本ライブラリでは、エンティティを[アセンブリ](./models/assembly_ja.md)の階層に載せることができます。エンティティのワールド座標は、定義空間の点にエンティティのDE変換（および上記のexplicit/implicit caseの変換鎖）を適用し、さらにルートから所有ノードまでのアセンブリ大域変換の積を外側に掛けたものになります。

　この合成順序は、Solid Assembly (Type 184) における「各アイテムの個別変換を適用した後に全体の大域変換を適用する」規則 (Section 3.3.2)、およびSubfigureのインスタンス化における入れ子処理の順序 (Section 3.6.2) と一致します。アセンブリ大域変換は、変換鎖の外側（左）に積まれます。

　フレーム指定や配置を適用した幾何問い合わせのAPIについては、[座標フレームと変換ビュー](./models/coordinate_frames_ja.md)を参照してください。

### アセンブリとビューの永続化

　アセンブリおよび変換ビューはメモリ上の構造であり、IGESファイルへは出力しません。既定では、読み込んだエンティティをフラットにそのまま出力するため、ラウンドトリップの忠実性が保たれます。

　アセンブリの配置をファイルへ永続化する場合は、大域変換をエンティティのDE変換 (Type 124) へ畳み込みます (Flatten / Materialize)。詳細は[座標フレームと変換ビュー](./models/coordinate_frames_ja.md)を参照してください。

...
> *1: グラフィックスモジュールはOpenGL4.3以降のcoreプロファイルコンテキストを要求しますが、macOSではOpenGL4.1までのサポートとなっているため、グラフィックス機能は利用できません（初期化時にエラーとなります）。グラフィックス以外の機能は制限なく利用できます
